...
const PainelFinanceiro = ({props}) => {
  ...
  const handlePrint = useReactToPrint({
	...
	const finalizaMesa = () => {
		...
		var div_imprime_cupom = document.getElementById("imprime_cupom")
		var div_finaliza_cupom = document.getElementById("cupom_finaliza_mesa")
		var div_finaliza_delivery = document.getElementById("cupom_finaliza_delivery")
		...
		imprime_cupom.addEventListener("click", () => {
			if (event.target.className == "bg_container_imprime_cupom" ) {
				...
				div_finaliza_delivery.classList.add("hide");
			}
		});
	}
	...
	return (
		<div className="container_painel_financeiro_pdv">
			...
			<div className="painel_operacional">
				...
				<div className="container_total_consumo" id="container_total_consumo">
						...
						{props.state.pagamentos_parciais.map(resp => {
							...
							return(
								<li className="pgt_parcial" key={resp.id} >
									...
									<span>
										{valor}
										<button onClick={function () {delPgtParc(resp.id)}}>
			...
			<div className="finalizar_painel_operacional">
				<button className="finaliza_caixa off" id="finaliza_caixa" onClick={() => {finalizaMesa()}} disabled={props.state.btn_finalizar_state}>Finalizar</button>
			</div>
